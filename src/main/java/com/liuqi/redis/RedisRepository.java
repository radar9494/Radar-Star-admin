...
public class RedisRepository {
     ...
     */
    public boolean setAndCompareGt(String key,Long newValue) {
        try {
    ...
    public boolean set(String key, Object value) {
        try {
            ...
            //获取过期时间
            long time=getExpire(key,TimeUnit.SECONDS);
            if(time>0){
     ...
     * @return
     */
    public boolean lRSet(String key, Object value, long time, TimeUnit unit) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0) {
                expire(key, time, unit);
     ...
     * @return
     */
    public boolean lLSet(String key, Object value, long time, TimeUnit unit) {
        try {
            redisTemplate.opsForList().leftPush(key, value);
            if (time > 0) {
                expire(key, time, unit);
     ...
     * @return 490
     */
    public boolean lRSet(String key, List<Object> value, long time, TimeUnit unit) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) {
                expire(key, time, unit);
     ...
     * @return 490
     */
    public boolean lLSet(String key, List<Object> value, long time, TimeUnit unit) {
        try {
            redisTemplate.opsForList().leftPushAll(key, value);
            if (time > 0) {
                expire(key, time, unit);
     ...
     * @param end
     */
    public void lRemove(String key,Long start,Long end){
        redisTemplate.opsForList().trim(key, start, end);
    }


    public void idPush(String key,Long id){
        redisTemplate.opsForList().leftPush(key,id);
    }
    public void idPushList(String key,Long ...value){
        redisTemplate.opsForList().leftPushAll(key,value);
    }
    public Long idPull(String key){
        Long id=0L;
        int count=4;
        while(count>0){
            count--;
    ...
    /******Zset***************************************************************************************************************/
    //添加
    public void add(String key, Object value, double d) {
        redisTemplate.opsForZSet().add(key, value, d);
    }

    //根据分数获取
    public Set getByScore(String key, double d1, double d2) {
        return redisTemplate.opsForZSet().rangeByScore(key, d1, d2);
    }
